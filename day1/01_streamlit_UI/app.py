...
# ダーク／ライトモード切り替え（今回追加）
theme = st.sidebar.selectbox("テーマを選択", ["ライト", "ダーク"])
if theme == "ダーク":
    st.markdown(
        """
        <style>
            .main { background-color: #2E2E2E; color: #EEE; }
        </style>
        """,
...
# ラジオボタン（今回追加）
st.subheader("クイズ：好きな機能は？")
choices = ["テキスト入力", "マップ表示", "ジョーク取得", "お絵描き"]
answer = st.radio("選んでください", choices)
if st.button("送信"):
    st.success(f"あなたの好きな機能は「{answer}」ですね！")
...
st.subheader("カウントダウンタイマー")
minutes = st.number_input("カウントダウン（秒）", min_value=5, max_value=60, value=10, step=5)
if st.button("スタート"):
    placeholder = st.empty()
    for i in range(minutes, -1, -1):
        placeholder.markdown(f"⏳ **残り {i} 秒**")
        time.sleep(1)
    placeholder.markdown("✅ **時間です！**")
    st.balloons()
